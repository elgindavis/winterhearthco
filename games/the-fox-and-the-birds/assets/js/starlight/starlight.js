...
var user_configuration = {
  ...
  final_size: "10px", //final size of the stars after expansion
  expand_speed: "0.2s", //how fast the stars get bigger, in seconds
  fade_delay: "0.5s", //how long until the star fades out
  fade_duration: "0.2s", //how long the star fades for
  colors: [
    "hsla(55, 89%, 89%, 1.0)",
    "hsl(0, 0%, 100%, 1.0)",
    "hsla(0, 0%, 99%, 1.0)",
  ], //The variety of colors of the stars. Can be any CSS complient color (eg. HEX, rgba, hsl)
  frequency: 70, //how often a new wave of stars pop-out (in milliseconds. Bigger==longer)
...
function Star(width, height) {
  ...
  this.xposition =
    Math.floor(Math.random() * width * user_configuration.coverage) +
    leftOffset;
  this.yposition =
    Math.floor(Math.random() * height * user_configuration.coverage) +
    topOffset;
}
...
Star.prototype.create = function (parent_element, id) {
  ...
  setTimeout(function () {
    star_container.css({
      //size expand properties
      transition:
        "height " +
        user_configuration.expand_speed +
        " " +
        advanced_configuration.expand_transition_timing +
        " " +
        advanced_configuration.expand_delay +
        "," +
        "width " +
        user_configuration.expand_speed +
        " " +
    ...
    if (user_configuration.rotation) {
      ...
      var transition =
        advanced_configuration.rotation_duration +
        " " +
        advanced_configuration.rotation_transition_timing +
        " " +
        advanced_configuration.rotation_delay;
    }
    ...
    if (!user_configuration.keep_lit) {
      ...
      if (transition) {
        transition +=
          ",opacity " +
          user_configuration.fade_duration +
          " " +
          advanced_configuration.fade_transition_timing +
          " " +
      ...
      }, css_time_to_milliseconds(user_configuration.fade_duration) +
        css_time_to_milliseconds(user_configuration.fade_delay));
    }
  ...
  if (
    user_configuration.custom_svg === "" ||
    user_configuration.custom_svg === false
  ) {
    star.css(
      "background-color",
      user_configuration.colors[
        Math.floor(Math.random() * user_configuration.colors.length)
      ]
    ); //picks one of the colors
  }
...
$(document).ready(function () {
  ...
  $(user_configuration.target_class).each(function (index) {
    ...
    setInterval(function () {
      //creates the stars based on the frequency and desired density
